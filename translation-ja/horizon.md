...
`auto`戦略を使う場合、Horizonがスケールアップ／ダウンで使用すべきプロセス数の最小値と最大値をコントロールするために、`minProcesses`と`maxProcesses`設定オプションを定義してください。`minProcesses`値はキューごとの最低プロセス数を指定します。`maxProcesses`値は全キュー合計の最大プロセス数を指定します。
