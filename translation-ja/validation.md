...
このルールは内部でPHPの`preg_match`関数を使用しています。パターンは有効なデリミタを使用していることも含め、`preg_match`が求めているフォーマットに従って指定する必要があります。
...
このルールは内部でPHPの`preg_match`関数を使用しています。パターンは有効なデリミタを使用していることも含め、`preg_match`が求めているフォーマットに従って指定する必要があります。たとえば：`'email' => 'regex:/^.+@.+$/i'`
