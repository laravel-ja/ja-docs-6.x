...
リリース直後はブランチ名が6.0だったのですが、リリース後にバージョニングの方針転換を反映したのか6.xがメンテ対象となり、公式ページのドキュメントURLも6.xへ変更されました。

ReaDoubleは、リンク切れを防ぐために一度公開したURLを削除しない方針です。そのため、Webサーバー側の設定を変更し、6.0以下のページへのアクセスは6.xへリダイレクトするように対応しています。
